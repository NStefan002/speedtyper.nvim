...
describe("Util test", function()
    ...
    it("utf_char_at", function()
        local str = "ćabšđ"
        eq("ć", util.utf_char_at(str, 1))
        eq("a", util.utf_char_at(str, 2))
        eq("b", util.utf_char_at(str, 3))
        eq("š", util.utf_char_at(str, 4))
        eq("đ", util.utf_char_at(str, 5))
        eq("", util.utf_char_at(str, 6))
        eq("", util.utf_char_at(str, 0))
        eq("đ", util.utf_char_at(str, -1))
        eq("š", util.utf_char_at(str, -2))
    end)

    it("utf_find", function()
        local str = "ćabšđ"
        eq(1, util.utf_find(str, "ć"))
        eq(2, util.utf_find(str, "a"))
        eq(3, util.utf_find(str, "b"))
        eq(4, util.utf_find(str, "š"))
        eq(5, util.utf_find(str, "đ"))
        eq(-1, util.utf_find(str, "ć", 2))
        eq(-1, util.utf_find(str, "đđ"))
        eq(-1, util.utf_find(str, "z"))
    end)

    it("utf_sub", function()
        local str = "ćabšđ"
        eq("ć", util.utf_sub(str, 1, 1))
        eq("đ", util.utf_sub(str, -1))
        eq("ab", util.utf_sub(str, 2, 3))
        eq("šđ", util.utf_sub(str, 4, 5))
        eq("đ", util.utf_sub(str, 5, 5))
        eq("đ", util.utf_sub(str, 5, 6))
        eq("", util.utf_sub(str, 6))
        eq("ćabšđ", util.utf_sub(str, 1))
        eq("ćabš", util.utf_sub(str, 1, -2))
    end)
end)
